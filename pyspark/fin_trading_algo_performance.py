...
class AlgorithmicTradingPerformance:
    ...
    def execute_pipeline(self, spark: SparkSession, run_date: str):
        ...
        # HDFS path replaced with BigQuery table reference. Replace 'your_gcp_project_id' and 'your_bigquery_dataset'.
        all_order_events_df = spark.read.format("bigquery").option("table", "your_gcp_project_id.your_bigquery_dataset.trading_events_base").load()
        # HDFS path replaced with BigQuery table reference. Replace 'your_gcp_project_id' and 'your_bigquery_dataset'.
        parent_orders_df = spark.read.format("bigquery").option("table", "your_gcp_project_id.your_bigquery_dataset.parent_orders").load()
        ...
        # HDFS path replaced with BigQuery table reference. Replace 'your_gcp_project_id' and 'your_bigquery_dataset'.
        quotes_df = spark.read.format("bigquery").option("table", "your_gcp_project_id.your_bigquery_dataset.level1_quotes").load()
        ...
        # HDFS path replaced with BigQuery table reference. Replace 'your_gcp_project_id' and 'your_bigquery_dataset'.
        trades_df = spark.read.format("bigquery").option("table", "your_gcp_project_id.your_bigquery_dataset.market_trades").load()
        ...
        # HDFS path converted to BigQuery table. Replace 'your_gcp_project_id' and 'your_bigquery_dataset'.
        # BigQuery table names do not allow hyphens, so 'run_date' is formatted to use underscores.
        final_df.write.format("bigquery").option("table", f"your_gcp_project_id.your_bigquery_dataset.trading_analytics_{run_date.replace('-', '_')}").mode("overwrite").save()
...
if __name__ == "__main__":
    ...
    # Configure SparkSession for BigQuery connectivity.
    # Add 'spark-bigquery-with-dependencies' package to enable BigQuery interaction.
    # Set 'google.cloud.project' to your GCP project ID and 'temporaryGcsBucket' for staging.
    spark = (SparkSession.builder
        ...
        .config("google.cloud.project", "your_gcp_project_id")
