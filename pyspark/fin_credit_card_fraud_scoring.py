...
def score_transactions_for_fraud(spark, execution_date):
    ...
    # 1. Load Data
    full_cc_trx = spark.read.format("bigquery").option("table", "your_gcp_project_id.fin_core_dataset.cc_transactions").load()
    accounts = spark.read.format("bigquery").option("table", "your_gcp_project_id.fin_core_dataset.dim_accounts").load()
    merchants = spark.read.format("bigquery").option("table", "your_gcp_project_id.fin_core_dataset.dim_merchants").load()
    ...
    final_output.write \
        .format("bigquery") \
        .option("table", "your_gcp_project_id.fin_mart_dataset.fraud_scores_daily") \
