...
def get_spark_session():
    """Initializes and returns a Spark session with BigQuery support enabled."""
...
def process_customer_onboarding(spark):
    ...
    """
    # 1. Read Raw Data sources from Google Cloud Storage
    raw_customers_df = spark.read.json("gs://migconvert-at-next26-work-bkt/landing/fin/customers/")
    raw_kyc_df = spark.read.parquet("gs://migconvert-at-next26-work-bkt/landing/fin/kyc_status/")
    raw_accounts_df = spark.read.csv("gs://migconvert-at-next26-work-bkt/landing/fin/accounts/", header=True, inferSchema=True)
    ...
    final_dim_customers = enriched_customer_df.select(
        ...
        # Create a dedicated date column for BigQuery date partitioning
        "bq_partition_date", to_date(col("last_account_open_date"))
    )

    # 6. Write to BigQuery Table
    # The target is migconvert-at-next26.fin_core.dim_customers, partitioned by bq_partition_date
    final_dim_customers.write \
        .format("bigquery") \
        .option("table", "migconvert-at-next26.fin_core.dim_customers") \
        .option("temporaryGcsBucket", "migconvert-at-next26-work-bkt/spark_temp") \
        .option("partitionField", "bq_partition_date") \
        .mode("overwrite") \
        .save()
