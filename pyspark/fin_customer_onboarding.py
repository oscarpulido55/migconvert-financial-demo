...
def get_spark_session():
    """Initializes and returns a Spark session with BigQuery support enabled.""" # Updated docstring for BigQuery
    return SparkSession.builder \
        ...
        .config("gcp.project", "my-gcp-project-id") \
...
def process_customer_onboarding(spark):
    ...
    """ # Updated docstring to reflect GCS/BigQuery
    # 1. Read Raw Data sources (Simulated paths in GCS) # Updated comment to GCS
    raw_customers_df = spark.read.json("gs://my-gcs-bucket/landing/fin/customers/") # Converted HDFS path to GCS path.
    raw_kyc_df = spark.read.parquet("gs://my-gcs-bucket/landing/fin/kyc_status/") # Converted HDFS path to GCS path.
    raw_accounts_df = spark.read.csv("gs://my-gcs-bucket/landing/fin/accounts/", header=True, inferSchema=True) # Converted HDFS path to GCS path.
    ...
    # The target is my-gcp-project-id.fin_core.dim_customers partitioned by onboarding_year, onboarding_month, country_code
    final_dim_customers.write \
        .partitionBy("onboarding_year", "onboarding_month", "country_code") \
        .format("bigquery") \
        .option("table", "my-gcp-project-id.fin_core.dim_customers") \
        .mode("overwrite") \
        .save()
...
if __name__ == "__main__":
    ...
    # Optional: Ensure BigQuery dataset exists. Note: BigQuery datasets (like 'fin_core') are typically managed outside Spark.
    # spark.sql("CREATE DATABASE IF NOT EXISTS fin_core") # Removed HiveQL database creation, as it's not applicable to BigQuery.
