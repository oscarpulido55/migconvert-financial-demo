...
def process_customer_onboarding(spark):
    ...
    raw_customers_df = spark.read.json("gs://your-gcs-bucket-name/landing/fin/customers/")
    raw_kyc_df = spark.read.parquet("gs://your-gcs-bucket-name/landing/fin/kyc_status/")
    raw_accounts_df = spark.read.csv("gs://your-gcs-bucket-name/landing/fin/accounts/", header=True, inferSchema=True)
    ...
    final_dim_customers.write \
        .format("bigquery") \
        .option("temporaryGcsBucket", "your-gcs-bucket-name-for-temp-files") \
        .option("table", "your_gcp_project.fin_core.dim_customers") \
        .option("partitionField", "onboarding_year") \
        .option("clusteringFields", "onboarding_month,country_code") \
        .mode("overwrite") \
        .save()
...
if __name__ == "__main__":
    ...
    spark.sql("CREATE SCHEMA IF NOT EXISTS `your_gcp_project`.fin_core")
