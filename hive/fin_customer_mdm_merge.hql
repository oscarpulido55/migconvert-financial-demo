-- ==============================================================================
-- Module: fin_customer_mdm_merge.hql -> fin_customer_mdm_merge.sql
-- Description: Master Data Management (MDM) module performing a Slowly Changing
-- Dimension (SCD) Type 2 upsert operation purely through SQL using FULL OUTER
...
-- BigQuery doesn't have a 'default' dataset, assuming `mydataset` or using unqualified names if default project set.
-- Using `mydataset` for demonstration; adjust as per project configuration.
DROP VIEW IF EXISTS `mydataset.vw_scd_transform_stg`;

CREATE VIEW `mydataset.vw_scd_transform_stg` AS
...
incoming_updates AS (
    ...
    FROM (
        ...
        WHERE DATE(timestamp) = '2024-01-01' -- Hive's to_date() is DATE() in BigQuery
    ) t WHERE rn = 1
)
...
SELECT
    ...
    FALSE AS is_active
FROM `mydataset.vw_scd_transform_stg`
...
SELECT
    ...
    GENERATE_UUID() AS customer_surrogate_key, -- Hive's reflect("java.util.UUID", "randomUUID") is BigQuery's GENERATE_UUID()
    customer_id,
    first_name_new,
    last_name_new,
    email_address_new,
