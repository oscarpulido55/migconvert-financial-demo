...
CREATE TABLE IF NOT EXISTS `fin_core.dim_customers_scd2` (
    ...
    is_active BOOL -- HIVEQL BOOLEAN converted to BIGQUERY BOOL
); -- Removed STORED AS ORC as it's not applicable in BigQuery CREATE TABLE DDL

-- View logic to do an SCD 2 transform over yesterday's active snapshot vs today's delta update
DROP VIEW IF EXISTS `default.vw_scd_transform_stg`; -- Added backticks for BigQuery object referencing

CREATE VIEW `default.vw_scd_transform_stg` AS -- Added backticks for BigQuery object referencing
...
incoming_updates AS (
    ...
    FROM (
        ...
        WHERE DATE(timestamp) = '2024-01-01' -- HIVEQL to_date(timestamp) converted to BIGQUERY DATE(timestamp)
    ) t WHERE rn = 1
)
...
INSERT INTO `fin_core.dim_customers_scd2` -- Removed TABLE keyword, added backticks
SELECT
    old_sk,
    customer_id,
    first_name_old,
    ...
    false AS is_active
FROM `default.vw_scd_transform_stg` -- Added backticks for BigQuery object referencing
WHERE change_type = 'UPDATE';

-- 2. Insert the completely NEW rows, and the NEW ACTIVE instances of updated rows
INSERT INTO `fin_core.dim_customers_scd2` -- Removed TABLE keyword, added backticks
SELECT
    GENERATE_UUID() AS customer_surrogate_key, -- HIVEQL reflect("java.util.UUID", "randomUUID") converted to BIGQUERY GENERATE_UUID()
    customer_id,
    first_name_new,
    last_name_new,
    email_address_new,
