...
-- 2. Create a temporary table to hold today's net movements per account
DROP TABLE IF EXISTS fin_core.tmp_daily_movements_stg;

CREATE TABLE fin_core.tmp_daily_movements_stg AS
WITH credited AS (
    SELECT
...
-- 3. Calculate End of Day Balances and Overwrite the specific date partition
...
LEFT JOIN fin_core.tmp_daily_movements_stg m
