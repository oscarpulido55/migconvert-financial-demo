...
-- In BigQuery, datasets are equivalent to databases and are typically created
-- outside of SQL DDL scripts, e.g., via `bq mk fin_core` or client libraries.
...
CREATE TABLE IF NOT EXISTS fin_core.fact_daily_balances (
    ...
    balance_date DATE OPTIONS(description='Date of the balance record, used for partitioning'),
    region_code STRING OPTIONS(description='Geographic region of the account, used for clustering')
)
PARTITION BY balance_date
CLUSTER BY region_code
OPTIONS(
    description='Stores the End of Day balances for all accounts',
    -- For date-partitioned tables, BigQuery requires `partition_filter_required`
    -- Set to false to allow queries without a WHERE clause on the partitioning column.
    partition_filter_required=false
);


-- 2. Temporary table to hold today's net movements per account
-- In BigQuery, temporary tables are session-scoped and do not require a dataset qualifier
-- if operating within the default dataset, or are implicitly scoped.
...
-- In BigQuery, `INSERT OVERWRITE PARTITION` for dynamically determined partitions
-- is typically achieved by a `DELETE` followed by an `INSERT INTO`.
-- This will delete all data for '2024-01-01' across all regions and insert the new data.
DELETE FROM fin_core.fact_daily_balances
WHERE balance_date = CAST('2024-01-01' AS DATE);
...
LEFT JOIN fin_core.fact_daily_balances prev
    ...
    -- BigQuery uses INTERVAL for date arithmetic.
