...
def run_etl_pipeline(spark: SparkSession, process_date: str):
    ...
    insert_sql = f"""
        CREATE OR REPLACE TABLE fin_core.fact_transactions
        PARTITION BY trx_date, region_id AS
        SELECT
            r.trx_uuid,
            r.source_account_id,
    ...
    risk_sql = f"""
        CREATE OR REPLACE TABLE fin_mart.risk_daily_summary
        ...
        SELECT
            ...
            PARSE_DATE('%Y-%m-%d', '{process_date}') AS summary_date
        FROM fin_core.fact_transactions
        WHERE trx_date = PARSE_DATE('%Y-%m-%d', '{process_date}')
        GROUP BY 
            customer_id,
            customer_segment,
            region_id
