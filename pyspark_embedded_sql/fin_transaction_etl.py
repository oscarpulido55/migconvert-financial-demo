/Users/oscarpulido/temp/migconvert_tmp_git_in_7416c8bb/pyspark_embedded_sql/fin_transaction_etl.py
import sys
import logging
...
def create_spark_session():
    """Initializes Spark Session with BigQuery compatibility."""
    # HIVEQL: The following Spark configurations are specific to Hive.
    # BIGQUERY: BigQuery typically manages partitioning internally based on table definitions
    # and doesn't use these specific runtime configurations.
    return SparkSession.builder \
        .appName("Financial_Transaction_ETL_Embedded_SQL") \
        .config("spark.sql.extensions", "com.google.cloud.spark.bigquery.SparkBigQueryExtensions") \
...
def run_etl_pipeline(spark: SparkSession, process_date: str):
    ...
    # HIVEQL: SET hiveconf:process_date is a Hive-specific way to set a session variable.
    # BIGQUERY: Removed as BigQuery does not use hiveconf, and direct f-string interpolation
    # is used for process_date in the SQL queries.
    ...
    spark.sql(f"""
        ...
        WHERE CAST(transaction_timestamp AS DATE) = '{process_date}' -- HIVEQL: to_date(timestamp_expression) -> BIGQUERY: CAST(timestamp_expression AS DATE)
    """)

    # 2. Complex ETL to fact table using embedded SQL
    logger.info("Inserting data into fin_core.fact_transactions...")
    insert_sql = f"""
        INSERT OVERWRITE TABLE fin_core.fact_transactions -- HIVEQL: PARTITION (trx_date, region_id) clause removed as BigQuery partitions are defined in DDL, not INSERT statements.
        SELECT
            r.trx_uuid,
            r.source_account_id,
            ...
            CAST(r.transaction_timestamp AS DATE) AS trx_date, -- HIVEQL: to_date(timestamp_expression) -> BIGQUERY: CAST(timestamp_expression AS DATE)
    ...
    risk_sql = f"""
        INSERT OVERWRITE TABLE fin_mart.risk_daily_summary -- HIVEQL: PARTITION (summary_date) clause removed as BigQuery partitions are defined in DDL, not INSERT statements.
        SELECT
            customer_id,
            customer_segment,
            region_id,
            ...
            '{process_date}' AS summary_date -- HIVEQL: ${hiveconf:process_date} replaced with direct f-string interpolation {process_date}
        FROM fin_core.fact_transactions
        WHERE trx_date = '{process_date}' -- HIVEQL: ${hiveconf:process_date} replaced with direct f-string interpolation {process_date}
        GROUP BY
            customer_id,
            customer_segment,
            region_id
...
if __name__ == "__main__":
    ...
    # HIVEQL: CREATE DATABASE IF NOT EXISTS <db_name>
    # BIGQUERY: CREATE SCHEMA IF NOT EXISTS <dataset_name>
