...
def create_spark_session():
    """Initializes Spark Session configured for BigQuery connector."""
    return SparkSession.builder \
        .appName("Financial_Transaction_ETL_Embedded_SQL") \
        .config("spark.jars", "gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-0.34.0.jar") \
...
def run_etl_pipeline(spark: SparkSession, process_date: str):
    ...
    # Parameters are now directly interpolated into SQL for BigQuery via Python.
    ...
    # BigQuery equivalent for INSERT OVERWRITE into a partitioned table (by trx_date) is usually DELETE then INSERT.
    spark.sql(f"""
        DELETE FROM `fin_core.fact_transactions` WHERE trx_date = DATE('{process_date}')
    """)
    insert_sql = f"""
    ...
    # BigQuery equivalent for INSERT OVERWRITE into a partitioned table (by summary_date) is usually DELETE then INSERT.
    spark.sql(f"""
        DELETE FROM `fin_mart.risk_daily_summary` WHERE summary_date = DATE('{process_date}')
    """)
    risk_sql = f"""
...
if __name__ == "__main__":
    ...
    # Initialize Datasets (BigQuery equivalent of databases) for safety
    # The 'project_id' is typically configured in the SparkSession or GCS connector.
