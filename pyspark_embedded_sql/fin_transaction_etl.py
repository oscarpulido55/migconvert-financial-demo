...
def create_spark_session():
    ...
    # Major Change: Replaced Hive-specific configurations with BigQuery connector settings.
    # Configuration parameters like `parentProject` and `temporaryGcsBucket` must be updated
    # with your actual GCP project ID and a GCS bucket for staging.
    # The `spark.jars.packages` dependency for the BigQuery connector is crucial.
    # Authentication (e.g., service account key path) might also be needed depending on your environment.
    """
...
def run_etl_pipeline(spark: SparkSession, process_date: str):
    ...
    # Major Change: Removed Hive-specific 'SET hiveconf' command.
    # Process date is now directly embedded into SQL queries using Python f-strings.
...
if __name__ == "__main__":
    ...
    # Major Change: Converted CREATE DATABASE to CREATE SCHEMA for BigQuery datasets.
    # Dataset names are now fully qualified with the GCP project ID.
    sp.sql("CREATE SCHEMA IF NOT EXISTS `your-gcp-project-id.fin_landing`")
    sp.sql("CREATE SCHEMA IF NOT EXISTS `your-gcp-project-id.fin_core`")
    sp.sql("CREATE SCHEMA IF NOT EXISTS `your-gcp-project-id.fin_mart`")
