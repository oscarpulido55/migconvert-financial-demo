...
def create_spark_session():
    ...
    return SparkSession.builder \
        ...
        .config("spark.datasource.bigquery.temporaryGcsBucket", "your-gcs-bucket-for-spark-bigquery-connector") \
        .config("gcp.project", "your-gcp-project-id") \
        .getOrCreate()

def run_etl_pipeline(spark: SparkSession, process_date: str):
    """
    Executes the ETL pipeline using complex embedded SQL queries, now targeting BigQuery.
    """
    logger.info(f"Starting execution for process_date: {process_date}")

    # Parameters are typically passed directly via Python string formatting for BigQuery.
    ...
    insert_sql = f"""
        CREATE OR REPLACE TABLE `your-gcp-project-id.fin_core.fact_transactions`
        AS
        SELECT
            r.trx_uuid,
            r.source_account_id,
    ...
    risk_sql = f"""
        CREATE OR REPLACE TABLE `your-gcp-project-id.fin_mart.risk_daily_summary`
        AS
        SELECT
            customer_id,
            customer_segment,
...
if __name__ == "__main__":
    ...
    # BigQuery datasets are typically pre-created. These commands are Hive-specific.
